...
class PublicChatController extends Controller
{
    ...
    public function chat(Request $request): JsonResponse
    {
        ...
        if (! $bot) {
            return $this->json(['error' => 'Invalid or inactive API key'], 404);
        }

        if (! $this->isAllowedDomain($request, $bot)) {
            return $this->json(['error' => 'This domain is not allowed for this chatbot.'], 403);
        }
        ...
        $context = DocumentChunk::query()
            ->where('chatbot_id', $bot->id)
            ->whereRaw("tsv @@ plainto_tsquery('english', ?)", [$data['message']])
            ->orderByRaw("ts_rank(tsv, plainto_tsquery('english', ?)) desc", [$data['message']])
            ->limit(4)
            ->pluck('content')
            ->implode("\n\n---\n\n");
        ...
        $system = trim($bot->system_prompt . "\n\n"
            ...
            . "When that happens, answer using the CURRENT PAGE CONTEXT first.\n"
            ...
            . ($context
                ...
                : "No knowledge base context matched. Answer from the current page context when available, otherwise say when you do not know."));
...
(function(){
  ...
  var ORIGIN = s && s.src ? new URL(s.src, window.location.href).origin : window.location.origin;
